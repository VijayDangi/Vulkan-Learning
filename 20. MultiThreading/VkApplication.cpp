...
// #define VK_FN_CHECK(x) \
//         {   \
//             VkResult result = x;    \
//             if(result != VK_SUCCESS)   \
//             {   \
//                 LogError(#x " Failed: %s", VulkanHelper::GetVulkanErrorCodeString(result)); \
//             }   \
//         }
...
namespace VkApplication
{
    struct Particle
    {
        ...
        {
            // code
            return { /*binding*/ 0, /*stride*/ sizeof(Particle), /*inputRate*/ VK_VERTEX_INPUT_RATE_VERTEX };
        }
        ...
        static std::array<VkVertexInputAttributeDescription, 2> getAttributeDescriptions()
        {
            ...
            return {
                VkVertexInputAttributeDescription{ 0, 0, VK_FORMAT_R32G32_SFLOAT, offsetof(Particle, position) },
                VkVertexInputAttributeDescription{ 1, 0, VK_FORMAT_R32G32B32A32_SFLOAT, offsetof(Particle, color) }
            };
        }
    };

    const uint32_t PARTICLE_COUNT = 256 * 256 * 4;
}
...
public:
    bool createThreadCommandPools(VkDevice& device, uint32_t queueFamilyIndex, uint32_t threadCount)
    {
        ...
        {
            VkResult result = vkCreateCommandPool(device, &poolInfo, nullptr, &commandPools[i]);
            if(result != VK_SUCCESS)
            {
                LogError("vkCreateCommandPool() Failed. %s", VulkanHelper::GetVulkanErrorCodeString(result));
                return false;
            }
        }
    ...
    VkCommandPool& getCommandPool(uint32_t threadIndex)
    {
        std::lock_guard<std::mutex> lock(resourceMutex);
        return commandPools[threadIndex];
    }

    bool allocateCommandBuffers(VkDevice& device, uint32_t threadCount, uint32_t buffersPerThread)
    {
        std::lock_guard<std::mutex> lock(resourceMutex);
        ...
        {
            LogError("Not enough command pools for thread count.");
            return false;
        }
        ...
        for( uint32_t i = 0; i < threadCount; ++i)
        {
            ...
            allocInfo.commandBufferCount = buffersPerThread;

            std::vector<VkCommandBuffer> buffers(buffersPerThread);

            VkResult result = vkAllocateCommandBuffers(device, &allocInfo, buffers.data());
            if(result != VK_SUCCESS)
            {
                LogError("vkAllocateCommandBuffers() Failed. %s", VulkanHelper::GetVulkanErrorCodeString(result));
                return false;
            }

            for(VkCommandBuffer& cmdBuffer : buffers)
            {
...
#define INVALID_QUEUE_FAMILY_HANDLE -1
    ...
    {
        uint32_t graphicsAndComputeFamily = INVALID_QUEUE_FAMILY_HANDLE;
        ...
        {
            // code
            return graphicsAndComputeFamily != INVALID_QUEUE_FAMILY_HANDLE &&
    ...
    */
    ...
    const std::vector<const char*> vulkanRequiredDeviceExtensions = { VK_KHR_SWAPCHAIN_EXTENSION_NAME, VK_KHR_TIMELINE_SEMAPHORE_EXTENSION_NAME };
    ...
    VkDescriptorPool vulkanComputeDescriptorPool;
    VkDescriptorSet vulkanComputeDescriptorSets[MAX_FRAMES_IN_FLIGHT]; // Don't need to explicitly cleanup descriptor sets, becaise it will auto freed when desciptor pool destroyed.
    
    VkDescriptorSetLayout vulkanComputeDescriptorSetLayout;
    VkPipelineLayout vulkanComputePipelineLayout;
    VkPipeline vulkanComputePipeline;
    
    // Compute Uniform Buffer
    VkBuffer vulkanComputeUniformBuffers[MAX_FRAMES_IN_FLIGHT];
    VkDeviceMemory vulkanComputeUniformBuffersMemory[MAX_FRAMES_IN_FLIGHT];
    ...
    static bool CreateVulkanInstance()
    {
        ...
        vulkanErrorCode = vkEnumerateInstanceVersion(&instanceVersion);
        if(vulkanErrorCode != VK_SUCCESS)
        {
            LogError("vkEnumerateInstanceVersion() Failed.");
            return false;
        }

        VulkanInstanceVersion.Major = VK_API_VERSION_MAJOR(instanceVersion);
        VulkanInstanceVersion.Minor = VK_API_VERSION_MINOR(instanceVersion);
        VulkanInstanceVersion.Patch = VK_API_VERSION_PATCH(instanceVersion);
        ...
        */
        ...
        appInfo.apiVersion = VK_MAKE_API_VERSION(0, VulkanInstanceVersion.Major, VulkanInstanceVersion.Minor, VulkanInstanceVersion.Patch);
    ...
    static QueueFamilyIndices FindQueueFamilies(VkPhysicalDevice device)
    {
        ...
        {
            // Check if Device has Graphics queue and Compute queue .
            ...
            {
                LogInfo("Graphics and Compute Queue Family Index: %u", queueIndices.graphicsAndComputeFamily);
    ...
    static bool PickVulkanPhysicalDevice()
    {
        ...
        for(const VkPhysicalDevice& device : physicalDevices)
        {
            if(IsPhysicalDeviceSuitable(device))
            {
                ...
                msaaImage.samplesCount = GetMaxUsableSampleCount();

                LogInfo("Vulkan: Selected Physical Device Name - \"%s\", with sample count: %d", deviceProperties.deviceName, (int)msaaImage.samplesCount);
                break;
            }
        }
    ...
    static bool CreateVulkanLogicalDevice()
    {
        ...
        graphicsQueueCreateInfo.queueFamilyIndex = indices.graphicsAndComputeFamily;
        ...
        vkGetDeviceQueue(vulkanLogicalDevice, indices.graphicsAndComputeFamily, 0, &vulkanQueue);
...
#undef max  // Doing this as max() macro is define in Windows.h
    ...
    */
    static bool CreateVulkanImageViewsForSwapchain()
    {
        ...
        {
            if(!CreateImageView(swapchain.images[imageIndex], swapchain.format, VK_IMAGE_ASPECT_COLOR_BIT, 1, &swapchain.imageViews[imageIndex]))
            {
...
////////////// Color Attachment Description.
        ...
        colorAttachmentDescrition.samples = msaaImage.samplesCount;
...
////////////// Depth Attachment Description.
        ...
        depthAttachmentDescription.samples        = msaaImage.samplesCount;
...
////////////// Color Resolve Attachment Description.
        ...
        colorAttachmentResolveDescription.format         = swapchain.format;
...
////////////// Render Pass
    ...
    */
    static bool CreateVulkanDescriptorSetLayout()
    {
        ...
        VkResult errorCode = vkCreateDescriptorSetLayout( vulkanLogicalDevice, &layoutInfo, nullptr, &graphicsPipeline.descriptorSetLayout);
        if(errorCode != VK_SUCCESS)
        {
...
////////////// Vertex Input
        ...
        std::array<VkVertexInputAttributeDescription, 2> attributeDescription = Particle::getAttributeDescriptions();
...
////////////// Input Assembly
        ...
        inputAssemblyCreateInfo.topology = VK_PRIMITIVE_TOPOLOGY_POINT_LIST; //VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST;
...
////////////// Multisampling
        ...
        multisampleCreateInfo.rasterizationSamples = msaaImage.samplesCount;
...
////////////// Pipeline Layout
        ...
        pipelineLayoutCreateInfo.pSetLayouts = &graphicsPipeline.descriptorSetLayout;
        pipelineLayoutCreateInfo.pushConstantRangeCount = 0;    // Optional
        pipelineLayoutCreateInfo.pPushConstantRanges = nullptr; // Optional

        VkResult errorCode = vkCreatePipelineLayout(vulkanLogicalDevice, &pipelineLayoutCreateInfo, nullptr, &graphicsPipeline.pipelineLayout);
        if(errorCode)
        {
...
////////////// Create Graphics Pipeline
        ...
        graphicsPipelineCreateInfo.pDynamicState = &dynamicStateCreateInfo;
            ...
            // pipeline layout
        graphicsPipelineCreateInfo.layout = graphicsPipeline.pipelineLayout;
        ...
        errorCode = vkCreateGraphicsPipelines( vulkanLogicalDevice, VK_NULL_HANDLE, 1, &graphicsPipelineCreateInfo, nullptr, &graphicsPipeline.pipeline);
        if(errorCode)
        {
    ...
    */
    static bool CreateVulkanFramebuffersForSwapchain()
    {
        ...
        for(size_t i = 0; i < swapchain.imageViews.size(); ++i)
        {
            ...
            VkImageView attachments[] = { msaaImage.colorImageView, depthBuffer.imageView, swapchain.imageViews[i]};
    ...
    static void CopyBuffer(VkBuffer sourceBuffer, VkBuffer destinationBuffer, VkDeviceSize size)
    {
        // code
            // Start Recording Command
        VkCommandBuffer commandBuffer = BeginSingleTimeCommands(vulkanCommandPool);
        ...
        vkCmdCopyBuffer( commandBuffer, sourceBuffer, destinationBuffer, 1, &copyRegion);
            ...
            // End Recording Command
        EndSingleTimeCommands(vulkanCommandPool, commandBuffer, vulkanQueue);
    ...
    static bool CreateImage2D(uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory)
    {
        ...
        imageInfo.sharingMode   = VK_SHARING_MODE_EXCLUSIVE;    // Image will only be used by one queue family: the one that supports graphics (and therefore also) transfer operations.
        imageInfo.samples       = numSamples;
        imageInfo.flags         = 0;

        QueueFamilyIndices indices = FindQueueFamilies(vulkanPhysicalDevice);
        uint32_t queuFamilyIndices[] = { indices.graphicsAndComputeFamily};
    ...
    static bool GenerateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels)
    {
        ...
        EndSingleTimeCommands(vulkanCommandPool, commandBuffer, vulkanQueue);   // vkCmdBlitImage() must be submitted to a queue with graphics capability.
    ...
    static bool CreateColorResources()
    {
        ...
        if(!CreateImage2D( swapchain.extent.width, swapchain.extent.height, 1, msaaImage.samplesCount, colorFormat,
            VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT,
            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, msaaImage.colorImage, msaaImage.colorImageMemory))
        {
            LogError("[Error] CreateImage2D() Failed.");
            return false;
        }

        if(!CreateImageView(msaaImage.colorImage, colorFormat, VK_IMAGE_ASPECT_COLOR_BIT, 1, &msaaImage.colorImageView))
        {
    ...
    static bool CreateDepthResources()
    {
        ...
        if(!CreateImage2D(
            swapchain.extent.width, swapchain.extent.height, 1, msaaImage.samplesCount, depthFormat,
            VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT,
            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depthBuffer.image, depthBuffer.imageMemory))
        {
            LogError("[Error] CreateImage2D() Failed.");
            return false;
        }
        
        if(!CreateImageView(depthBuffer.image, depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT, 1, &depthBuffer.imageView))
        {
            LogError("[Error] CreateImageView() Failed.");
            return false;
        }

        if(!TransitionImageLayout(depthBuffer.image, depthFormat, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL, 1))
        {
...
/////////////////////////////////////////////////////////////////////////
// ________________ CREATION OF COMPUTE PIPELINE START ________________ 
/////////////////////////////////////////////////////////////////////////

    void InitThreadResources()
    {
        QueueFamilyIndices queueFamilyIndices = FindQueueFamilies(vulkanPhysicalDevice);
        resourceManager.createThreadCommandPools(vulkanLogicalDevice, queueFamilyIndices.graphicsAndComputeFamily, threadCount);
        resourceManager.allocateCommandBuffers(vulkanLogicalDevice, threadCount, 1);
    }

    void RecordComputeCommandBuffer(VkCommandBuffer cmdBuffer, uint32_t startIndex, uint32_t count)
    {
        // code
        ...
        VkResult errorCode = vkBeginCommandBuffer( cmdBuffer, &beginInfo);
        if(errorCode)
        {
        ...
        vkCmdBindPipeline(cmdBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, vulkanComputePipeline);
        vkCmdBindDescriptorSets( cmdBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, vulkanComputePipelineLayout, 0, 1, &vulkanComputeDescriptorSets[currentFrame], 0, nullptr);

        struct PushConstant
        {
            uint32_t startIndex;
            uint32_t count;
        } pushConstant{ startIndex, count};
        vkCmdPushConstants(cmdBuffer, vulkanComputePipelineLayout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(PushConstant), &pushConstant);
    ...
    void WorkerThreadFunc(uint32_t threadIndex)
    {
        while(!shouldExit)
        {
            // Wait for work using condition variable
            {
                ...
                workCompleteCv.wait( lock, [threadIndex](){
            ...
            VkCommandBuffer *cmdBuffer = &resourceManager.getCommandBuffer(threadIndex);
            ...
            // Notify main therad and otther threads
            {
    ...
    void InitThreads()
    {
        ...
        threadCount = 2u;
        LogInfo("Initializing %u threads for sequential execution.", threadCount);
        ...
        for(uint32_t i = 0; i < threadCount; ++i)
        {
            ...
            LogInfo(
                "Thread %u will process particles %u to %u (count: %u)",
                i,
        ...
        for(uint32_t i = 0; i < threadCount; ++i)
        {
            workerThreads.emplace_back(&WorkerThreadFunc, i);
            LogInfo("Started worker thread %u", i);
        }
    }
    ...
    void SignalThreadsToWork()
    {
        ...
        // Memroy barrier to ensure all threads see the updated threadWorkDone values.
        std::atomic_thread_fence(std::memory_order_seq_cst);

        // Only signal the first thread to start work
        threadWorkReady[0].store(true, std::memory_order_release);

         // Notify all threads in case they're waiting on the coditional variable
        {
            std::lock_guard<std::mutex> lock(workCompleteMutex);
            workCompleteCv.notify_all();
        }
    ...
    void WaitForThreadsToComplete()
    {
        ...
        bool waitResult = workCompleteCv.wait_for(lock, std::chrono::milliseconds(3000), []() {
    ...
    /**
     * @brief CreateComputeVulkanDescriptorSetLayout()
    */
    static bool CreateComputeVulkanDescriptorSetLayout()
    {
        // code
        VkDescriptorSetLayoutBinding computeLayoutBindings[3]{};
        ...
        computeLayoutInfo.bindingCount = _ARRAYSIZE(computeLayoutBindings);
        computeLayoutInfo.pBindings = computeLayoutBindings;
        
        VkResult errorCode = vkCreateDescriptorSetLayout( vulkanLogicalDevice, &computeLayoutInfo, nullptr, &vulkanComputeDescriptorSetLayout);
        if(errorCode != VK_SUCCESS)
        {
            LogError("[Error] Failed to create descriptor set layout. %s", VulkanHelper::GetVulkanErrorCodeString(errorCode));
            return false;
    ...
    /**
     * @brief CreateComputeDescriptorPool()
     */
    static bool CreateComputeDescriptorPool()
    {
        // code
        VkDescriptorPoolSize poolSizes[2]{};
        poolSizes[0].type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
        ...
        VkResult errorCode = vkCreateDescriptorPool(vulkanLogicalDevice, &poolInfo, nullptr, &vulkanComputeDescriptorPool);
        if(errorCode)
        {
            LogError("[Error] vkCreateDescriptorPool() Failed. %s", VulkanHelper::GetVulkanErrorCodeString(errorCode));
            return false;
    ...
    /**
     * @brief CreateComputeDescriptorSets()
     */
    static bool CreateComputeDescriptorSets()
    {
        // code
        VkDescriptorSetLayout layouts[MAX_FRAMES_IN_FLIGHT];
        for( int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
        {
            layouts[i] = vulkanComputeDescriptorSetLayout;
        }

        VkDescriptorSetAllocateInfo allocInfo{};
        allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
        allocInfo.descriptorPool = vulkanComputeDescriptorPool;
        allocInfo.descriptorSetCount = static_cast<uint32_t>(MAX_FRAMES_IN_FLIGHT);
        allocInfo.pSetLayouts = layouts;
        
        VkResult errorCode = vkAllocateDescriptorSets(vulkanLogicalDevice, &allocInfo, vulkanComputeDescriptorSets);
        if(errorCode)
        {
            Log("vkAllocateDescriptorSets() Failed. %s", VulkanHelper::GetVulkanErrorCodeString(errorCode));
            return false;
        ...
        for( size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
        {
            ...
            uniformBufferInfo.buffer = vulkanComputeUniformBuffers[i];
            uniformBufferInfo.offset = 0;
            uniformBufferInfo.range = sizeof(ComputeUniformBufferObject);

            descriptorWrite[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
            descriptorWrite[0].dstSet = vulkanComputeDescriptorSets[i];
            descriptorWrite[0].dstBinding = 0;
            descriptorWrite[0].dstArrayElement = 0;
            descriptorWrite[0].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
            descriptorWrite[0].descriptorCount = 1;
            ...
            descriptorWrite[1].dstSet = vulkanComputeDescriptorSets[i];
            ...
            descriptorWrite[2].dstSet = vulkanComputeDescriptorSets[i];
    ...
    /**
     * @brief CreateVulkanComputePipeline()
    */
    static bool CreateVulkanComputePipeline()
    {
        // code
        ////////////// Loading a shader.
        std::vector<char> computeShaderCode;

        if(!ReadFile("shaders/compute.comp.spv", computeShaderCode))
        {
            return false;
        }

        ////////////// Creating shader modules.
        VkShaderModule computeShaderModule = CreateVulkanShaderModule(computeShaderCode);
        if(!computeShaderModule)
        {
            LogError("[Error] CreateVulkanShaderModule() Failed.");
            return false;
        }

        // Create Push Constant range for particle
        ...
        pipelineLayoutCreateInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
        pipelineLayoutCreateInfo.setLayoutCount = 1;
        pipelineLayoutCreateInfo.pSetLayouts = &vulkanComputeDescriptorSetLayout;
        ...
        VkResult errorCode = vkCreatePipelineLayout(vulkanLogicalDevice, &pipelineLayoutCreateInfo, nullptr, &vulkanComputePipelineLayout);
        if(errorCode)
        {
            LogError("Vulkan: [Error] vkCreatePipelineLayput() Failed. %s", VulkanHelper::GetVulkanErrorCodeString(errorCode));
            return false;
        ...
        // pipeline layout
        computePipelineCreateInfo.layout = vulkanComputePipelineLayout;

        errorCode = vkCreateComputePipelines( vulkanLogicalDevice, VK_NULL_HANDLE, 1, &computePipelineCreateInfo, nullptr, &vulkanComputePipeline);
        if(errorCode)
        {
            LogError("Vulkan: [Error] vkCreateComputePipelines() Failed. %s", VulkanHelper::GetVulkanErrorCodeString(errorCode));
            return false;
        }
    ...
    /**
     * @brief CreateShaderStorageBuffers()
     */
    static bool CreateShaderStorageBuffers()
    {
        // code
        ...
        for(Particle& particle : particles)
        {
            ...
            float velocityMagnitude = MAX(minVelocity, r * velocityScale);
            particle.velocity = glm::normalize( glm::vec2(x, y)) * velocityMagnitude;

            if(particle.position.x > 0 && particle.position.y > 0)
            {
                particle.color = glm::vec4(0.8f, 0.0f, 0.0f, 1.0f);
            }
            else if(particle.position.x < 0 && particle.position.y > 0)
            {
                particle.color = glm::vec4(0.0f, 0.8f, 0.0f, 1.0f);
            }
            else if(particle.position.x < 0 && particle.position.y < 0)
            {
                particle.color = glm::vec4(0.0f, 0.0f, 0.8f, 1.0f);
            }
            else
            {
                particle.color = glm::vec4(0.8f, 0.8f, 0.8f, 1.0f);
            }
        ...
        for( size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
        {
            if(!CreateBuffer( bufferSize, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, shaderStorageBuffers[i], shaderStorageBuffersMemory[i]))
            {
                LogError("[Error] CreateBuffer() Failed.");
                return false;
            }
        ...
        LogSuccess("Vulkan: [Success] Create Storage Buffer and Memory allocated.");

        return true;
    }

    /**
     * @brief CreateComputeUniformBuffer()
     */
    static bool CreateComputeUniformBuffer()
    {
        // code
        ///////////////////////////////////////////////
        ////////////////////// Compute Uniform Buffer
        ...
        for(size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
        {
            if(!CreateBuffer(bufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, vulkanComputeUniformBuffers[i], vulkanComputeUniformBuffersMemory[i]))
            {
                LogError("[Error] CreateBuffer() Failed for Uniform at %d.", (int)i);
                return false;
            }

            vkMapMemory(vulkanLogicalDevice, vulkanComputeUniformBuffersMemory[i], 0, bufferSize, 0, &computeUniformBuffersMapped[i]);
        }
        return true;
    }
    ...
    static bool CreateVulkanSyncObjects()
    {
        ...
        VkSemaphoreCreateInfo timeSemaphoreInfo{};
        timeSemaphoreInfo.sType = VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO;
        timeSemaphoreInfo.pNext = &semaphoreType;
        timelineValue = 0;

        errorCode = vkCreateSemaphore(vulkanLogicalDevice, &timeSemaphoreInfo, nullptr, &timelineSemaphore);
        if(errorCode)
        {
...
/////////////////////////////////////////////////////////////////////////
// ________________ CREATION OF COMPUTE PIPELINE END ________________ 
/////////////////////////////////////////////////////////////////////////
...
#define CHECK_FUNCTION_RETURN(x) \
        ...
        CHECK_FUNCTION_RETURN(CreateVulkanCommandPool(VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT, queueFamilyIndices.graphicsAndComputeFamily, vulkanCommandPool));
        ...
        CHECK_FUNCTION_RETURN(CreateVulkanCommandBuffers(vulkanCommandPool));

        /////////////////////////// Initialize Compute Pipeline
        // Create Compute Descriptor Set Layout
        CHECK_FUNCTION_RETURN(CreateComputeVulkanDescriptorSetLayout());

        // Create Compute Pipeline
        CHECK_FUNCTION_RETURN(CreateVulkanComputePipeline());

        // Create Shader Storage Buffer
        CHECK_FUNCTION_RETURN(CreateShaderStorageBuffers());

        // Create Compute Uniform Buffer
        CHECK_FUNCTION_RETURN(CreateComputeUniformBuffer());

        // Create Compute Descriptor Pool
        CHECK_FUNCTION_RETURN(CreateComputeDescriptorPool());

        // Create Compute Descriptor Sets
        CHECK_FUNCTION_RETURN(CreateComputeDescriptorSets());

        InitThreads();

        // Create Compute Command Buffers
        // CHECK_FUNCTION_RETURN(CreateVulkanComputeCommandBuffers(vulkanCommandPool));
...
#undef CHECK_FUNCTION_RETURN
    ...
    static void CleanupSwapChain()
    {
        ...
        {
            vkDestroyImageView(vulkanLogicalDevice, depthBuffer.imageView, nullptr);
            depthBuffer.imageView = nullptr;
        }

        if(depthBuffer.image)
        {
            vkDestroyImage(vulkanLogicalDevice, depthBuffer.image, nullptr);
            depthBuffer.image = nullptr;
        }

        if(depthBuffer.imageMemory)
        {
            vkFreeMemory(vulkanLogicalDevice, depthBuffer.imageMemory, nullptr);
            depthBuffer.imageMemory = nullptr;
        }

        if(msaaImage.colorImageView)
        {
            vkDestroyImageView(vulkanLogicalDevice, msaaImage.colorImageView, nullptr);
            msaaImage.colorImageView = nullptr;
        }

        if(msaaImage.colorImage)
        {
            vkDestroyImage(vulkanLogicalDevice, msaaImage.colorImage, nullptr);
            msaaImage.colorImage = nullptr;
        }

        if(msaaImage.colorImageMemory)
        {
            vkFreeMemory(vulkanLogicalDevice, msaaImage.colorImageMemory, nullptr);
            msaaImage.colorImageMemory = nullptr;
        }
    }
...
////////////// Drawing Commands
        ...
        // Bind Descriptor
        vkCmdBindDescriptorSets( commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, graphicsPipeline.pipelineLayout, 0, 1, &vulkanDescriptorSets[currentFrame], 0, nullptr);
...
////////////// Finishing Up
     ...
     * @brief RecordCommandBuffer()
     */
    static bool RecordWholeComputeCommandBuffer(VkCommandBuffer commandBuffer)
    {
        // code
            // We always begin recording a command buffer by calling 'vkBeginCommandBuffer()'.
        VkCommandBufferBeginInfo beginInfo{};
        ...
        // Bind Descriptor
        vkCmdBindDescriptorSets( commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, vulkanComputePipelineLayout, 0, 1, &vulkanComputeDescriptorSets[currentFrame], 0, nullptr);
    ...
    static void UpdateUniformBuffer(uint32_t currentImage)
    {
        ...
        ubo.modelMatrix = glm::identity<glm::mat4>();// glm::rotate(glm::mat4(1.0f), time * glm::radians(10.0f), glm::vec3(0.0f, 0.0f, 1.0f));
        ubo.viewMatrix = glm::identity<glm::mat4>();// glm::lookAt(glm::vec3(2.0f, 2.0f, 2.0), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 0.0f, 1.0f));
        ubo.projectionMatrix = glm::identity<glm::mat4>();//glm::perspective(glm::radians(45.0f), vulkanSwapChainExtent.width / (float)vulkanSwapChainExtent.height, 0.1f, 10.0f);
        ...
        //////////////
        ComputeUniformBufferObject uboCompute{};
        uboCompute.deltaTime = deltaTime;
        deltaTime = 0.3f;
        
        // copy data to uniform buffer
        memcpy(computeUniformBuffersMapped[currentImage], &uboCompute, sizeof(uboCompute));
    }
    ...
    void DrawFrame()
    {
        ...
        // Signal worker threads to to start processing particles
        SignalThreadsToWork();
        ...
        computeSubmitInfo.commandBufferCount = computeCmdBuffers.size();
        computeSubmitInfo.pCommandBuffers = computeCmdBuffers.data();  // Command Buffers to actually submit for execution.
        computeSubmitInfo.waitSemaphoreCount = 1;
        computeSubmitInfo.pWaitSemaphores = &timelineSemaphore;
        computeSubmitInfo.pWaitDstStageMask = waitStages;
        computeSubmitInfo.signalSemaphoreCount = 1;
        computeSubmitInfo.pSignalSemaphores = &timelineSemaphore;    // Specify which semaphores to signal once the command buffer(s) have finished execution.

        {
            std::lock_guard<std::mutex> lock(queueSubmitMutex);
            VkResult errorCode = vkQueueSubmit( vulkanQueue, 1, &computeSubmitInfo, nullptr);
            if(errorCode)
            {
        ...
        std::array<VkSemaphore, 2> waitSemaphores = { timelineSemaphore, imageAvailableSemaphores[currentFrame]};
        std::array<uint64_t, 2> waitSemaphoresValues = { graphicsWaitValue, 0};
        ...
        graphicsTimelineInfo.waitSemaphoreValueCount = waitSemaphoresValues.size();
        graphicsTimelineInfo.pWaitSemaphoreValues = waitSemaphoresValues.data();
        ...
        graphicsSubmitInfo.waitSemaphoreCount = waitSemaphores.size();
        graphicsSubmitInfo.pWaitSemaphores = waitSemaphores.data();    // Specify which semaphores to wait on before execution begins.
        graphicsSubmitInfo.pWaitDstStageMask = graphicsWaitStages;      // In which stage(s) of the pipeline to wait.
            // Each entry in the 'waitStage' array corresponds to the semaphore with the same index in 'pWaitSemaphores'
        graphicsSubmitInfo.commandBufferCount = 1;
        graphicsSubmitInfo.pCommandBuffers = &vulkanCommandBuffers[currentFrame];  // Command Buffers to actually submit for execution.
        graphicsSubmitInfo.signalSemaphoreCount = 1;
        graphicsSubmitInfo.pSignalSemaphores = &timelineSemaphore;    // Specify which semaphores to signal once the command buffer(s) have finished execution.

        {
            std::lock_guard<std::mutex> lock(queueSubmitMutex);
            VkResult errorCode = vkQueueSubmit( vulkanQueue, 1, &graphicsSubmitInfo, inFlightFences[currentFrame]);
            if(errorCode)
            {
                LogError("Vulkan: [Error] vkQueueSubmit() Failed. %s", VulkanHelper::GetVulkanErrorCodeString(errorCode));
                return;
        ...
        // ____________________________________ Presentation ___________________________________________________ //
        ...
        result = vkWaitSemaphores(vulkanLogicalDevice, &waitInfo, 5000000000);
        if(result == VK_TIMEOUT)
        {
            LogError("vkWaitSemaphore() Failed");
            vkDeviceWaitIdle(vulkanLogicalDevice);
            return;
        }
    ...
    /**
     * @brief ComputeVariableCleanup()
     */
    void ComputeVariableCleanup()
    {
        // code
        if(vulkanComputeDescriptorPool)
        {
            vkDestroyDescriptorPool(vulkanLogicalDevice, vulkanComputeDescriptorPool, nullptr);
            vulkanComputeDescriptorPool = nullptr;
        }

        if(vulkanComputeDescriptorSetLayout)
        {
            vkDestroyDescriptorSetLayout( vulkanLogicalDevice, vulkanComputeDescriptorSetLayout, nullptr);
            vulkanComputeDescriptorSetLayout = nullptr;
        }

        for(uint32_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
        {
            // SSBO
            if(shaderStorageBuffers[i])
            {
                vkDestroyBuffer(vulkanLogicalDevice, shaderStorageBuffers[i], nullptr);
                shaderStorageBuffers[i] = nullptr;
            }

            if(shaderStorageBuffersMemory[i])
            {
                vkFreeMemory(vulkanLogicalDevice, shaderStorageBuffersMemory[i], nullptr);
                shaderStorageBuffersMemory[i] = nullptr;
            }

            // Uniform
            if(vulkanComputeUniformBuffers[i])
            {
                vkDestroyBuffer(vulkanLogicalDevice, vulkanComputeUniformBuffers[i], nullptr);
                vulkanComputeUniformBuffers[i] = nullptr;
            }

            if(vulkanComputeUniformBuffersMemory[i])
            {
                vkUnmapMemory(vulkanLogicalDevice, vulkanComputeUniformBuffersMemory[i]);
                computeUniformBuffersMapped[i] = nullptr;

                vkFreeMemory(vulkanLogicalDevice, vulkanComputeUniformBuffersMemory[i], nullptr);
                vulkanComputeUniformBuffersMemory[i] = nullptr;
            }
        }
        ...
        if(vulkanComputePipeline)
        {
            ...
            vulkanComputePipeline = nullptr;
        }

        if(vulkanComputePipelineLayout)
        {
            vkDestroyPipelineLayout(vulkanLogicalDevice, vulkanComputePipelineLayout, nullptr);
            vulkanComputePipelineLayout = nullptr;
        }
    }
    ...
    // Wait for the logical device to finish operations before existing
        ...
        CleanupSwapChain();
        ComputeVariableCleanup();

        resourceManager.Destroy(vulkanLogicalDevice);
        ...
        {
            vkDestroySemaphore(vulkanLogicalDevice, timelineSemaphore, nullptr);
